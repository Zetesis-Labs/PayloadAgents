...
> **Enmienda 2026-07-09 (big-bang ZP)**: ZP ejecuta el endgame cero-Redis **directamente en el devcontainer** — M10 implementada (store KV + selector `idempotency_backend`, suite parametrizada por store, 34 asserts verdes en la matriz redis/nats/nats+kv) y validada e2e con Redis apagado. La secuencia dual-consume/drain (M4/M5) deja de aplicar a ZP: el patrón D9 (job en `CREATED` + sweep) hace la cola re-derivable desde Payload, el productor habla HTTP con el kicker, y el tráfico es esporádico — el paso a Kubernetes será una instalación NATS limpia + re-kick de docs pendientes, no un cutover con drain. La maquinaria dual-consume queda documentada para flotas con tráfico en vuelo (nixon), y el código v1 (taskiq/Redis) permanece en el paquete hasta que nixon migre.
> **Enmienda 2026-07-10 (retirada del kicker)**: tras pasar el cliente TS a publicar directo al broker (y con el v1 taskiq/Redis ya eliminado del paquete), la fachada HTTP `/enqueue` (kicker) se quedó sin consumidores y se retira — el HTTP del worker queda en probes/métricas (`create_probes_app`). Si aparece un productor que no pueda hablar NATS (webhook, serverless), se reintroduce con el contrato registrado en el spec §8.3.
...
- **No colas compartidas entre proyectos** — nunca. Interop de operaciones sí; si un proyecto necesita invocar trabajo en otro: una fachada HTTP con HMAC en el borde (retirada del paquete hasta que haga falta — spec §8.3) o, post-migración, import/export de subjects.
