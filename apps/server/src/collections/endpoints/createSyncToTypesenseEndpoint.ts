...
export const createSyncToTypesenseEndpoint = (collectionSlug: CollectionSlug): Endpoint => ({
  handler: async (req) => {
    ...
    try {
      ...
      if (!tableConfig) {
        throw new APIError(
          `${collectionNameCap} collection is not configured for indexing`,
          500,
          null,
          true
        );
      ...
      console.log(
        `[Typesense Sync] Starting sync of ${response.totalDocs} ${collectionName}...`
      );
      ...
      for (const doc of response.docs) {
        try {
          ...
          console.error(
            `[Typesense Sync] Error syncing ${collectionName.slice(0, -1)} ${doc.id}:`,
            error
          );
        }
      }
      ...
      console.log(
        `[Typesense Sync] Completed: ${results.synced.length} synced, ${results.errors.length} errors`
      );
