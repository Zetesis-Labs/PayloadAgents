...
export async function ensureStripeCustomer(
  ...
  const customerId = await getCustomerFromStripeOrCreate(email, user.name ?? undefined)
  const customer = typeof user.customer === 'object' ? user.customer : null
  const customerInventory = customer?.inventory as CustomerInventory | undefined
  await upsertCustomerInventoryAndSyncWithUser(payload, customerInventory, email, customerId, config.userSlug)
