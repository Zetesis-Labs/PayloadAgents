...
export function useChatSession(adapter: ChatAdapter): UseChatSessionReturn {
  ...
  useEffect(() => {
    const loadActiveSession = async () => {
      try {
        console.log(
          "[useChatSession] 🔄 Loading active session through adapter...",
        );
        ...
        if (sessionData) {
          console.log(
            "[useChatSession] ✅ Active session found:",
          ...
          if (sessionData.messages && sessionData.messages.length > 0) {
            ...
            console.log(
              "[useChatSession] ✅ Session restored with",
          ...
          console.log(
            "[useChatSession] ℹ️ No active session found (adapter returned null)",
          );
        }
      } catch (error) {
    ...
    async (id: string) => {
      try {
        ...
        if (sessionData) {
          ...
          console.error(
            "[useChatSession] ❌ Failed to load session (adapter returned null)",
          );
        }
      } catch (error) {
        console.error("[useChatSession] ❌ Error loading session:", error);
    ...
    async (id: string) => {
      try {
        ...
        if (success) {
          // Update local history
          setSessionsHistory((prev) =>
            prev.filter((s) => s.conversation_id !== id),
          );
