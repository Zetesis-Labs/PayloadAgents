...
consumes it — envelope intact end to end, on every transport.
...
from .harness import SECRET, Scratch, TransportHarness
...
async def _run_ts_producer(port: int) -> str:
    """Drive the real TS client against the kicker; returns its stdout."""
...
async def test_ts_producer_to_python_worker_roundtrip(
    ...
    # Worker first, so the consumer exists before the TS enqueue.
    async with (
        harness.running_worker(config, scratch, [HandlerSpec("test.xlang", xlang, EchoPayload)]),
        _serving(harness.make_kicker_app(config)) as port,
    ):
        await _run_ts_producer(port)
...
async def test_ts_producer_envelope_on_the_wire(harness: TransportHarness) -> None:
    ...
    async with _serving(harness.make_kicker_app(config)) as port:
        await _run_ts_producer(port)
